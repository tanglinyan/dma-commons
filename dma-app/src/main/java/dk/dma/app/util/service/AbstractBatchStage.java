 ...
 */
public abstract class AbstractBatchStage<T> extends AbstractMessageProcessorService<T> {

    private final int maxBatchSize;

    protected AbstractBatchStage(int queueSize, int maxBatchSize) {
        super(queueSize);
        if (maxBatchSize < 1) {
    ...
    protected abstract void handleMessages(List<T> messages);
