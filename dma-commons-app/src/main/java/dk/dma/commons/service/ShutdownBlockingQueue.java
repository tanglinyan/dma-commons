...
/**
 * An optionally-bounded {@linkplain BlockingQueue blocking queue} based on linked nodes. This queue orders elements
 * FIFO (first-in-first-out). The <em>head</em> of the queue is that element that has been on the queue the longest
 * time. The <em>tail</em> of the queue is that element that has been on the queue the shortest time. New elements are
...
public class ShutdownBlockingQueue<E> extends AbstractQueue<E> implements BlockingQueue<E>, java.io.Serializable {
    ...
    /*
     * A variant of the "two lock queue" algorithm. The putLock gates entry to put (and offer), and has an associated
     * condition for waiting puts. Similarly for the takeLock. The "count" field that they both rely on is maintained as
     * an atomic to avoid needing to get both locks in most cases. Also, to minimize need for puts to get takeLock and
     * vice-versa, cascading notifies are used. When a put notices that it has enabled at least one take, it signals
     * taker. That taker in turn signals others if more items have been entered since the signal. And symmetrically for
    ...
    private void checkLastElement() {
        if (capacity == 0 && count.get() == 0) {
            fullyShutdown.countDown();
        }
    }
     ...
     * {@code true} if this queue contained the specified element (or equivalently, if this queue changed as a result of
     * the call).
     * 
     * @param o
     ...
     * 
     * <p>
     * Suppose {@code x} is a queue known to contain only strings. The following code can be used to dump the queue into
     ...
     * 
     * @param a
     *            the array into which the elements of the queue are to be stored, if it is big enough; otherwise, a new
    ...
    private class Itr implements Iterator<E> {
         ...
         * true, we will still have it to return even if lost race with a take etc.
         */
